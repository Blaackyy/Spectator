        ...
        for (Player all : this.plugin.getSpectators()) {
            if (all != null) {
                ...
                if (this.plugin.getRelation().containsKey(all)) {
                    if (CycleHandler.isPlayerCycling(all))
                        msg = Messages.getMessage(Paths.MESSAGES_COMMANDS_LIST_CYCLING, "SPECTATOR", all.getDisplayName());
                    else
                        msg = Messages.getMessage(Paths.MESSAGES_COMMANDS_LIST_SPECTATING, "SPECTATOR", all.getDisplayName(), "TARGET", this.plugin.getRelation().get(all).getDisplayName());
                } else
                    msg = Messages.getMessage(Paths.MESSAGES_COMMANDS_LIST_DEFAULT, "SPECTATOR", all.getDisplayName());
